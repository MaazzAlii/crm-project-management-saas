...
## In Progress
- [ ] TASK 03 — Nginx Reverse Proxy, Domains & SSL Configuration
...
- `76ea337` — `feat(infra): deploy self-hosted Supabase stack via Docker Compose`

## Deployment Status
- Self-hosted Supabase Docker Compose definition (`docker-compose.supabase.yml`), Kong routing configuration (`infra/kong/kong.yml`), environment template (`.env.supabase.example`), and architecture guide (`documentation/infra/self-hosted-supabase.md`) committed.
...
`TASK 03 — Nginx Reverse Proxy, Domains & SSL Configuration`
(see `03-nginx-reverse-proxy-domains-and-ssl.md`)
