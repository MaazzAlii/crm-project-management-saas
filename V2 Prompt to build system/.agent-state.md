...
- `f8a1173` — `feat(infra): configure Nginx reverse proxy with SSL for all subdomains`

## Deployment Status
- Nginx server blocks created (`nginx/conf.d/app.conf`, `nginx/conf.d/api.conf`, `nginx/conf.d/n8n.conf`) and DNS/Let's Encrypt SSL guide created (`documentation/infra/dns-ssl.md`).
...
`TASK 04 — Backup & Disaster Recovery Strategy`
(see `04-backup-and-disaster-recovery-strategy.md`)
