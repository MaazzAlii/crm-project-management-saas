...
- [x] TASK 68 — CI/CD Pipeline to Contabo VPS (`chore(deploy): set up CI/CD pipeline deploying to the self-hosted Contabo VPS`)
- [x] TASK 69 — n8n Workflow Activation on Contabo VPS (`chore(deploy): activate n8n automation workflows in production`)

## In Progress
- [ ] TASK 70 — Monitoring, Logging, and Uptime for Self-Hosted Infrastructure
...
- CI/CD automated via GitHub Actions (.github/workflows/deploy.yml) pushing standalone Docker images to GHCR and deploying via SSH to Contabo VPS with automated /api/health check and rollback.
- Co-located n8n automation engine at n8n.innoventixhub.com on internal port 5678, with HMAC-SHA256 signature verification across all 4 workflows (invoice trigger, deadline alert, overdue alert, weekly summary).
...
- Production build (NODE_ENV=production npm run build) verified 100% clean with standalone output.
- Docker multi-stage build, .dockerignore, and docker-compose.app.yml configured.
- GitHub Actions CI/CD pipeline configured with fail-fast type-check/tests, GHCR image push, SSH deployment, post-deploy health check, and automated rollback.
- n8n production deployment documentation and verification procedures documented in documentation/infra/n8n-deployment.md.
...
`TASK 70 — Monitoring, Logging, and Uptime for Self-Hosted Infrastructure`
(see `70-monitoring-logging-and-uptime-for-self-hosted-infrastructure.md`)
